 ...
 */
export function GeneralHighlightSection(): JSX.Element {
  const { docRevision } = viewerState.use();
  const { mode, groups, hiddenKeys, monochrome, dimDrawing, customFilters } = highlightState.use();
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Checkbox
        ...
        label="Dim drawing"
        tooltip="Fade the drawing so the overlays stand out. Shared with Label Inspect and Node Positions — it sits below every overlay, so it never dims one of them"
      />
